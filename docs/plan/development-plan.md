...
| Task | Description | Duration | Dependencies | Technical Details |
|------|-------------|----------|--------------|-------------------|
| 2.1 Product Catalog | Build a scalable service for 50,000 SKUs with multilingual descriptions, basic search, dietary/expiry filters, popular items, and promotion integration. | 2.5 weeks | 1.4 | **Tech**: Django 4.2, PostgreSQL, `django-environ`, `django-filter`. **Models**: `Product(sku: str, name_en: str, name_vi: str, category_id: fk, price: decimal, is_perishable: bool, expires_at: datetime, stock_updated_at: datetime, dietary: array[str], tags: array[str])`, `Category(id: uuid, name_en: str, name_vi: str)`, `Promotion(id: uuid, type: str, percentage: decimal, valid_until: datetime, rules: jsonb)`. **API**: REST (`/products`, `/products/{sku}`, `/categories`, `/products/popular`, `/promotions`, GET/POST) with pagination, filtering (`category`, `is_perishable`, `dietary`, `expires_soon`, `tags`, `price_range`). **Search**: PostgreSQL GIN index on `name_en`, `name_vi`, `dietary`, `tags` for ≤500ms queries; sync with Elasticsearch (2.8). **Popular Items**: `SELECT sku, COUNT(*) FROM order_items WHERE created_at > now() - interval '7 days' GROUP BY sku ORDER BY COUNT DESC LIMIT 10`. **Promotions**: Support SKU-based, category-based, perishable-specific discounts (`rules: {"sku": "milk", "expires_days": 3}`). **Caching**: Redis (`products:{sku}`, `popular:{category}`, `promotions:{id}`, 1-hour TTL). **Rate Limiting**: NGINX (`limit_req zone=products burst=50`). **Health Check**: `/health`. **Kafka**: Publish `ProductUpdated`, `PromotionUpdated` to `product_events`. **Scripts**: Seed 50,000 SKUs (`seed_products.py`, 20% perishables, 10% vegan/gluten-free), promotions (`seed_promotions.py`). **Tests**: pytest for filters, popular items, promotions (≥85% coverage); Testcontainers for PostgreSQL. **Metrics**: Prometheus for filter usage, cache hits, promotion application. **Logging**: Log slow queries (>300ms) to Sentry. **Grocery**: Support dietary filters, near-expiry highlights, popular items, perishable promotions. **Scalability**: Shard PostgreSQL by `category_id`; index `dietary`, `tags`. **HA**: 2 replicas in `kind` via Helm (`charts/product-catalog`). **Debug**: Use Django Debug Toolbar locally. **CI/CD**: GitHub Actions (`lint`, `test`, `build`, `push`). **Docker**: Image (`grocery/product-catalog:1.0`, ~200MB). **Resource**: 512MB RAM. |
...
- **Product Catalog (API with Promotions and Filters)**:
  ...
  class Promotion(models.Model):
      ...
      type = models.CharField(max_length=20)  # e.g., 'sku', 'category', 'perishable'
      percentage = models.DecimalField(max_digits=5, decimal_places=2)
      valid_until = models.DateTimeField()
      rules = models.JSONField(default=dict)  # e.g., {"sku": "milk", "expires_days": 3}
  ```
  ```python
...
- **Recommendation (Collaborative Filtering)**:
  ...
  class RecommendationViewSet(viewsets.ReadOnlyModelViewSet):
      ...
      def get_queryset(self):
          ...
          # Rule-based: Suggest from user's preferred categories
